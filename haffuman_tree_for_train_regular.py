...
from haffuman import build_haffuman_tree_for_each_layer,\
                     ...
                     myindex
import vgg16_trainable_regular as vgg16
...
save_model              = 'trainedmodel_for_regular'

the_num_of_train_images = 22511
the_num_of_val_images   = 10000
inf_log                 = 'log_for_regular'
...
def loss(label_batches,logits):
    ...
    cost = tf.reduce_mean(cross_entropy)+ tf.add_n(tf.get_collection('losses')) 
    return cost
...
if __name__ == '__main__':
    ...
    '''
    name = ''
    if not os.path.exists(inf_log+'/0.txt'):
        open(inf_log+'/0.txt', 'w').close()
        name = inf_log+'/0.txt'
    else:
        L_name = []
        for root,dirs,files in os.walk(inf_log+'/'):
            for line in files:
    ...
    vgg.build(images, regularizer, train_mode)
    '''
    ...
    for i in range(layer_num):
        for List in layer_node[i]:
            ...
            vgg.mynetwork(vgg.relu7,name,len(List),regularizer)
